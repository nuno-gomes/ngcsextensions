...
namespace NunoGomes.CommunityServer.Captcha.Providers
{
  ...
  {
    private class SimpleCaptchaControl : PlaceHolder
    {
      ...
      void captchaPostValidator_ServerValidate(object source, ServerValidateEventArgs args)
      {
        ...
        if (string.IsNullOrEmpty(txtCaptcha) || string.IsNullOrEmpty(_cipherCode))
        {
          args.IsValid = false;
        }
        else
        {
          args.IsValid = (this._cipherCode == this._cipherProvider.Encrypt(txtCaptcha));
        }
      }
